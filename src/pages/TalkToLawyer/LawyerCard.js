...
const LawyerCard = ({ lawyer, specialtiesArray }) => {
  ...
  const date = new Date();
  return (
    <div className="bg-primary dark:bg-base-100   shadow flex flex-col h-full w-80 items-start justify-start rounded-xl gap-5 text-base-100 dark:text-primary dark:border border-gray-800 relative  ">
        ...
        <div className="flex flex-col ">
          ...
          <div className="grid grid-cols-3 gap-3">
              ...
              <div className="flex flex-col items-start justify-start">
                  ...
                  sortedSpecialties.slice(0, 6)?.map((item, index) => (
                    <span
                      className={`text-xs  m-1 p-1 rounded-md ${
                        matchedSpecialties.includes(item)
                          ? "bg-[#FFB2AE] text-primary"
                          : "bg-primary dark:bg-base-100"
                      }`}
                      key={index}
                    >
                      {item}
                ...
                {sortedSpecialties.length === 0 &&
                  specialties
                    ...
                    ?.slice(0, 6)
                    ?.map((item, index) => (
                      <span className="text-xs  m-1 p-1 rounded-md" key={index}>
                        {item}
                      </span>
                    ))}
              </div>
            ...
            <div className="flex flex-col items-end p-1  rounded-lg ">
              <span className="font-semibold my-1">Languages</span>
