                                    ...
                                    <li key={index} className={` flex flex-col items-start`}>
                                      <span className="pb-1 text-accent">{item.title}</span>                      
                                      {
                                        item.submenu.map((subItem, subIndex) => (
                                          <NavLink key={subIndex} to={subItem.link} className="transition-all duration-300 text-base-100 dark:text-primary w-fit flex items-center hover:text-accent hover:dark:text-accent bg-transparent">{subItem.title}</NavLink>
                                        ))
                                      }
                                    </li>
                                  ))
            ...
            <li> <NavLink to="" className={({ isActive }) =>isActive ? activeClassName : 'text-base-100 dark:text-primary w-fit flex items-center gap-2 p-2 hover:text-accent '}>Home</NavLink></li> 
              <li
                  className={`relative group flex gap-1 justify-center items-center transition-all duration-300 text-base-100 dark:text-primary bg-primary dark:bg-base-100  w-max cursor-pointer text-sm `}>
                      <NavLink to="/demo" className={({isActive}) => isActive ? activeClassName : 'text-base-100 dark:text-primary w-fit flex items-center gap-2 p-2 hover:text-accent group-hover:text-accent'}>Find Lawyer <FaAngleDown className="transition-all duration-300 group-hover:rotate-180 group-hover:text-accent" /> </NavLink>
                          ...
                          <div className="grid grid-cols-3 lg:grid-cols-5 lg:gap-2 2xl:gap-10 bg-primary dark:bg-base-100 p-1 lg:p-5 shadow rounded-lg">
                              {
                                findLawyer.map((item, index) => (
                                  <div key={index} className="flex flex-col gap-2">
                                    <p className="lg:text-xl border-b pb-3 text-accent">{item.title}</p>                      
                                    {
                                      item.submenu.map((subItem, subIndex) => (
                                        <NavLink key={subIndex} to={subItem.link} className="transition-all duration-300 text-base-100 dark:text-primary w-fit flex items-center gap-2 p-2 hover:text-accent hover:dark:text-accent">{subItem.title}</NavLink>
                                      ))
                                    }
                                  </div>
                                ))
              ...
              <li
                className={`relative group flex gap-1 justify-center items-center transition-all duration-300 text-base-100 dark:text-primary bg-primary dark:bg-base-100  w-max cursor-pointer text-sm `}>
                    <NavLink to="property-sale-and-purchase" className={({isActive}) => isActive ? activeClassName : 'text-base-100 dark:text-primary w-fit flex items-center gap-2 p-2 hover:text-accent group-hover:text-accent'}>Property <FaAngleDown className="transition-all duration-300 group-hover:rotate-180 group-hover:text-accent"  /></NavLink>
                        ...
                        <ul className='space-y-2 rounded-lg shadow z-50 bg-primary'>
                            {
                                allProperties.map((item, index) => (
                                  ...
                                  >
                                    <Link key={index} to={`${item.link}`}><li className={`flex items-center gap-3 transition-all duration-300 cursor-pointer px-5 py-4 z-50  hover:text-accent h-full w-max bg-primary dark:bg-base-100 ${menu === 'property' && 'text-accent'}`}>{item.name} <FaAngleRight /></li></Link>
                                    <div className={`w-max absolute -right-44 top-0 rounded-lg bg-primary dark:bg-base-100  ${item.submenu && menu === 'property' ? '' : 'hidden'}`}>
                                        <ul className='space-y-5 rounded-lg'>
                                            {
                                                item.submenu?.map((sub,index) => (
                                                  <Link 
                                                    key={index} className='' to={`${sub.link}`}>
                                                        <li className='transition-all duration-300 text-base-100 dark:text-primary w-max flex items-center gap-2 p-2 hover:text-accent hover:dark:text-accent '>
                                                        { sub.name}
                                                        </li>
                                                    </Link>
